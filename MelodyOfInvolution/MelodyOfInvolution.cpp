...
const char* MOI_TITLE = "Melody Of Involution [R1.2.1]";	//更新时修改此处Version
...
void play(char item[], int audio_config)
{
	...
	for (; i <= len_int; i++)
	{
		...
		if (pause + internal_upright > 0)	//避免Sleep(0)
		{
			Sleep(pause + internal_upright);	//pause = target_speed - consuming | TODO 更改为clock()计时
		}
	}
	t_end = clock();
	fclose(fsong);
